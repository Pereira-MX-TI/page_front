import { Component, OnInit,AfterViewInit,Renderer2,ViewChild,AfterContentInit, OnDestroy} from '@angular/core';
...
export class HomeComponent implements OnInit,AfterViewInit,OnDestroy {
  ...
  ngOnInit(): void 
  {
    ...
    this.renderer.listen(window,"resize",()=>{this.redimensionComponent();});
    this.refresh();
  ...
  panelState(state:boolean):void
  {
    ...
    if(state)
    {
      ...
      setTimeout(()=>{this.objNavegationService.setPositionScrooll(500);},150);
    }
    else
      this.messageBtnFooter="Mas informacion";
  }
  ...
  refresh()
  {
    ...
    this.objHttpService.getListCarousel({
      listId:[1,2,3]
    }).subscribe(
      res=>
      {
      ...
      err=>{
        this.objShareInformationService.viewLoading$.emit(false);
        this.objSnackBar.open("Error obtener carrusel",null,{duration: 2500,panelClass: ['snackBar_error']});
      }
    );
  }
  ...
  navegate(opc:number,search:string)
  {
    ...
    switch(opc)
    {
      case 0:this.objNavegationService.navegatePage("Product/List");break;

      case 1:
      {
        this.objLocalStorageService.save('selpro',this.objCryptoService.encrypted(search));
        this.objNavegationService.navegatePage("Product/List");
      }break;
    }
  }
