...
export class NavBarComponent implements OnInit,OnDestroy {
  ...
  private objWindowSizeService:WindowSizeService) 
  {
    ...
    this.listOption=this.objNavegationService.listSelectMenu(objRouter.url)[0];
  }
  ...
  ngOnInit(): void 
  {
    ...
    this.listSubscription[0] = this.objShareInformationService.search2$.subscribe((response:string[])=>{
      this.listObservable = response;
    });
    
    this.listSubscription[1] = this.objShareInformationService.selectMenuMovil$.subscribe( (response:boolean[]) => {
      this.listOption=response;
    });   

    this.listSubscription[2] = this.objShareInformationService.manageCarShopping$.subscribe( (response:any) => {
  ...
  mouseEnter(trigger:any,opcMenu:number):void 
  {
    switch(opcMenu)
    {
      case 0:this.hoverMenuTriggerRead.closeMenu();break;
    }
  ...
  {
    this.timedOutCloser = setTimeout(() => {trigger.closeMenu();}, 50);
  }
  ...
  returnWindow():void
  {
    ...
    urlCurrent = urlCurrent.includes("/DashBoard/Admin/Read/Consumption/Detail")?"/DashBoard/Admin/Read/Consumption/Detail":urlCurrent;
    urlCurrent = urlCurrent.includes("/DashBoard/Admin/Read/Schedule/Detail")?"/DashBoard/Admin/Read/Schedule/Detail":urlCurrent;
  ...
  applySearch():void
  {
    ...
    {
      this.dialog.open(ModalDialogComponent,{panelClass:'search',width: '100vw',
      maxWidth: '100vw',position:{top:'0',left:'0'},closeOnNavigation:true,autoFocus: false,data:{opcView:1}});
    }
  }
  ...
  manageShoppingCar(data:any,opc:number):void
  {
    switch(opc)
    {
      case 0:
      {
        ...
        if(insert)
          this.shoppingCar.push(data);
      }break;

      case 1:this.shoppingCar = data;break;
      case 2:this.shoppingCar = [];break;
    }
