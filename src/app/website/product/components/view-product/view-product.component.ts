...
export class ViewProductComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    this.currentProduct = this.objCryptoService.decrypted(
      atob(
  ...
  ngOnInit(): void {
    ...
    this.listSubscription[2] =
      this.objShareInformationService.reloadSamePage$.subscribe(
        (response: any) => {
    ...
    this.objHttpService
      ...
      .subscribe(
        (res) => {
          ...
          this.publicity = [
            data.publicity.listProduct1,
            data.publicity.listProduct2,
          ];

          this.viewCurrentImg =
            this.currentProduct.files.length != 0
              ? this.pipeProduct.transform(
                  this.currentProduct.files[0],
                  0,
                  'image2',
                )
              : './../../../../assets/default.jpg';
        ...
        (err) => {
          this.objShareInformationService.viewLoading$.emit(false);
          this.objSnackBar.open('Error obtener producto', null, {
            duration: 2500,
  ...
  addProduct(): void {
    if (this.formGroup.valid) {
      this.objShareInformationService.manageCarShopping$.emit({
        data: {
    ...
    } else this.formGroup.controls.quantity.markAsTouched();
  }
        ...
        if (response != undefined && response != null && response) {
          ...
          this.objHttpService
            ...
            .subscribe(
              ...
              (err) => {
                this.objShareInformationService.viewLoading$.emit(false);
                this.objSnackBar.open('Error obtener folio', null, {
                  duration: 2500,
                  panelClass: ['snackBar_error'],
                });
              },
