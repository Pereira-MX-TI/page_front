...
export class ShopCarProductComponent implements OnInit {
      ...
      this.shoppingCar.forEach((itrData) => {
        this.formGroup.push(
          new UntypedFormGroup({
            quantity: new UntypedFormControl(itrData.quantity, [
              Validators.required,
              Validators.min(1),
            ]),
          }),
  ...
  getPublicity() {
    let listId: any[] = [];
    this.shoppingCar.forEach((itrProduct) => {
      listId.push(itrProduct['product']['id']);
    });
    ...
    this.objHttpService.getPublicity({ listProduct: listId }).subscribe(
      ...
      (err) => {
        this.objShareInformationService.viewLoading$.emit(false);
        this.objSnackBar.open('Error obtener publicidad', null, {
          duration: 2500,
  ...
  changeQuantity(opc: number): void {
    if (this.formGroup[opc].valid) {
      this.dataSourceFilter.filteredData[opc].quantity =
        this.formGroup[opc].value['quantity'];
    ...
    } else this.formGroup[opc].controls.quantity.markAsTouched();
  }
