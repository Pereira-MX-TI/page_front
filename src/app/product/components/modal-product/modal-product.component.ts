...
export class ModalProductComponent implements OnInit {
    ...
    @Inject(MAT_DIALOG_DATA) public data:any) 
  {
    ...
    switch (this.case) 
    {
      ...
      {
        this.listFilter = [
        {title:'Categorias',list:this.data["filters"]['categories'],status:false,opcFilter:3},
        {title:'Marcas',list:this.data["filters"]['brands'],status:false,opcFilter:5},
        {title:'Materiales',list:this.data["filters"]['materials'],status:false,opcFilter:7}
        ];
      }break;
  ...
  setResponse(data:any): void
  {
    switch (this.case) 
    {
      case 0:this.dialogRef.close(data);break;
      case 1:this.dialogRef.close(data);break;
      case 2:
      {
        if(this.formGroup.valid)
        {
          const dialogRef = this.dialog.open(ModalDialogComponent,{
          closeOnNavigation:true,autoFocus: false,data:{listMessage:["Confirmar",
          "¿Desea continuar con la solicitud?","Aceptar","rgb(0, 0, 97)"],opcView:0}});   
